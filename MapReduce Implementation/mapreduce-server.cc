...
void MapReduceServer::spawnMappers() throw() {
  ThreadPool Thpool(kNumWorkerThreads);
  ...
  for(size_t i = 0; i < kNumWorkerThreads; i++) {
  	...
  	const string& mapperNode = mapperNodes[i % numServers];
  	string command = buildMapperCommand(mapperNode, mapperExecutable, intermediatePath);
  	Thpool.schedule([this, mapperNode, command]{spawnWorker(mapperNode, command);});  
  }
  Thpool.wait();
...
void MapReduceServer::spawnReducers() throw() {
  ...
  for (unsigned int i=0; i < numReducers*numMappers; i++){
	string newS;
        stringstream trans;
        trans << setw(5) << setfill('0') << i;
        trans >> newS;
	unprocessed.push_back(intermediatePath + "/*." + newS + ".mapped");
  }
  vector<string> reducerNodes(nodes);
  size_t numServers = reducerNodes.size();
  for(size_t i = 0; i < kNumWorkerThreads; i++) {
        random_shuffle(reducerNodes.begin(), reducerNodes.end());
        const string& reducerNode = reducerNodes[i % numServers];
        string command = buildReducerCommand(reducerNode, reducerExecutable, outputPath);
        Thpool.schedule([this, reducerNode, command]{spawnWorker(reducerNode, command);});
  }
  Thpool.wait();
...
string MapReduceServer::buildReducerCommand(const string& remoteHost,
                                           ...
                                           const string& outputPath) throw() {
  ostringstream oss;
  string reducerPath = reducer[0] == '/' ? reducer : cwd + "/" + reducer;
  oss << "ssh -o ConnectTimeout=5 " << user << "@" << remoteHost
      << " '" << reducerPath
      << " " << host
      << " " << serverPort
